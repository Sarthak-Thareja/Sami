...
export default function Navbar() {
  ...
  return (
    <motion.nav
      className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-lg rounded-3xl bg-creamy-white/70 backdrop-blur-md border border-rose-gold/20 shadow-lg px-2 py-2"
          ...
          return (
            <li key={href}>
              <Link href={href} className="relative block p-3 rounded-2xl" aria-label={label}>
                ...
                >
                  <Icon
                    className={`w-6 h-6 sm:w-7 sm:h-7 ${isActive ? "text-rose-gold fill-soft-blush" : "text-deep-berry/80"}`}
                  />
